...
def train_epoch_binary(model, loader, criterion, optimizer, device):
    ...
    # Initialize counters for FPR calculation
    tp, fp, tn, fn = 0, 0, 0, 0
    ...
    for batch_x, batch_y_binary, _ in loader:
        ...
        # Update the correct count for accuracy calculation
        correct += (predicted == batch_y_binary).sum().item()
        total += batch_y_binary.size(0)
        
        # Update counts for FPR and F1 calculation
        tp += ((predicted == 1) & (batch_y_binary == 1)).sum().item()  # True Positives
        tn += ((predicted == 0) & (batch_y_binary == 0)).sum().item()  # True Negatives
        fp += ((predicted == 1) & (batch_y_binary == 0)).sum().item()  # False Positives
        fn += ((predicted == 0) & (batch_y_binary == 1)).sum().item()  # False Negatives
...
def evaluate_binary(model, loader, criterion, device):
    ...
    with torch.no_grad():
        for batch_x, batch_y_binary, _ in loader:
            ...
            total_loss += loss.item()
            # Consistent threshold logic with training
    ...
    # Precision/Recall calculation for edge cases
...
def train_epoch_multiclass(model, loader, criterion, optimizer, device):
    ...
    # Initialize confusion matrix elements for FPR calculation
    num_classes = len(loader.dataset.classes)  # Assuming the loader provides class information
    cm = torch.zeros(num_classes, num_classes)  # Confusion matrix: rows=actual, cols=predicted
    ...
    for batch_x, batch_y in loader:
        ...
        # Store predictions and true labels for later F1 score calculation
        all_preds.extend(predicted.cpu().numpy())
        all_labels.extend(batch_y.cpu().numpy())
        
        # Update confusion matrix for FPR calculation
    ...
    # Calculate F1-score (macro or weighted based on your preference)
    f1 = f1_score(all_labels, all_preds, average='weighted', zero_division=0)

    # Calculate Combined FPR (False Positive Rate across all classes)
    total_fp = cm.sum(dim=0) - cm.diag()  # Sum of False Positives for each class
    total_tn = cm.sum() - cm.sum(dim=0) - cm.sum(dim=1) + cm.diag()  # Sum of True Negatives for each class
    combined_fpr = total_fp.sum() / (total_fp.sum() + total_tn.sum()) if (total_fp.sum() + total_tn.sum()) > 0 else 0.0

    return total_loss / len(loader), accuracy, f1, combined_fpr
def evaluate_multiclass(model, loader, criterion, device):
    ...
    with torch.no_grad():
        for batch_x, batch_y_attack in loader:
            ...
            all_preds.extend(predicted.cpu().numpy())
            all_labels.extend(batch_y_attack.cpu().numpy())
            
            # Update confusion matrix for FPR calculation
            for t, p in zip(batch_y_attack.view(-1), predicted.view(-1)):
                cm[t.long(), p.long()] += 1
    ...
    # Calculate combined FPR (False Positive Rate across all classes)
    total_fp = cm.sum(dim=0) - cm.diag()  # False Positives for each class
    total_tn = cm.sum() - cm.sum(dim=0) - cm.sum(dim=1) + cm.diag()  # True Negatives for each class
    combined_fpr = total_fp.sum() / (total_fp.sum() + total_tn.sum()) if (total_fp.sum() + total_tn.sum()) > 0 else 0.0
    
    avg_loss = total_loss / len(loader) if len(loader) > 0 else 0
    
    return avg_loss, accuracy, f1, combined_fpr


def train_epoch_joint(model, loader, binary_criterion, multiclass_criterion, optimizer, device):
    """
    Joint training function for binary and multiclass classification.

    Args:
        model: The neural network model
        loader: DataLoader containing the training data (with binary and attack labels)
    ...
    Returns:
        ...
        avg_accuracy: The average accuracy (combining binary and multiclass)
        avg_recall: The average recall (combining binary and multiclass)
        avg_precision: The average precision (combining binary and multiclass)
        avg_fpr: The average FPR (combining binary and multiclass)
        macro_f1: The macro average F1 score (combining binary and multiclass)
    """
    model.train()
    total_loss = 0
    binary_correct = 0
    ...
    tp, fp, tn, fn = 0, 0, 0, 0  # For binary classification
    multiclass_tp, multiclass_fp, multiclass_tn, multiclass_fn = 0, 0, 0, 0  # For multiclass classification
    ...
    for batch_x, batch_y_binary, batch_y_attack in loader:
        ...
        if attack_mask.sum() > 0:
            ...
            # Adaptive loss balancing - increase multiclass weight over time
        ...
        # Binary metrics
        predicted_binary = (binary_output > 0.0).float()  # Use 0.0 for logits
        binary_correct += (predicted_binary == batch_y_binary).sum().item()
        binary_total += batch_y_binary.size(0)
        
        # Confusion matrix for binary classification
        tp += ((predicted_binary == 1) & (batch_y_binary == 1)).sum().item()
        fp += ((predicted_binary == 1) & (batch_y_binary == 0)).sum().item()
        tn += ((predicted_binary == 0) & (batch_y_binary == 0)).sum().item()
        fn += ((predicted_binary == 0) & (batch_y_binary == 1)).sum().item()
        
        # Multiclass metrics
        _, predicted_multiclass = multiclass_output.max(1)
        multiclass_correct += predicted_multiclass.eq(batch_y_attack).sum().item()
        multiclass_total += batch_y_attack.size(0)
        
        for class_id in torch.unique(torch.cat([batch_y_attack, predicted_multiclass])):
            class_id = class_id.item()
            true_class = (batch_y_attack == class_id).float()
            pred_class = (predicted_multiclass == class_id).float()
    ...
    binary_fpr = fp / (fp + tn) if fp + tn > 0 else 0
    ...
    multiclass_precision = multiclass_tp / (multiclass_tp + multiclass_fp) if multiclass_tp + multiclass_fp > 0 else 0
    multiclass_recall = multiclass_tp / (multiclass_tp + multiclass_fn) if multiclass_tp + multiclass_fn > 0 else 0
    multiclass_fpr = multiclass_fp / (multiclass_fp + multiclass_tn) if multiclass_fp + multiclass_tn > 0 else 0
    multiclass_f1 = 2 * (multiclass_precision * multiclass_recall) / (multiclass_precision + multiclass_recall) if multiclass_precision + multiclass_recall > 0 else 0
    ...
    return avg_loss, avg_accuracy, avg_recall, avg_precision, avg_fpr, macro_f1
